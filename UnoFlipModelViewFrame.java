...
public class UnoFlipModelViewFrame extends JFrame implements UnoFlipModelView {
   ...
   // private UnoFlip game;
    ...
    public UnoFlipModelViewFrame() {
        ...
        model = new UnoFlip(player,deck);
        model.addUnoFlipView(this);
        direction= UnoFlip.Direction.FORWARD;
        ...
        drawButtonListener listener1 = new drawButtonListener(model,this,event,this.direction);
        drawCardButton.addActionListener(listener1);

        NextPlayerButtonListener listener = new NextPlayerButtonListener(model,event,this,this.direction);
        nextPlayerButton.addActionListener(listener);



        cardClickerListener listener2 = new cardClickerListener(model,event,this);
    ...
    public void updateButton(UnoFlipEvent e ) {
        Card card1= e.givePlayerCard();
        createCardButton(card1);
        buttonPanel.add(createCardButton(card));
        cardButtons.add(createCardButton(card));
    }
    ...
    public JButton createCardButton(Card card) {
        ...
        if (card.getCardType() == Card.CardType.NUMBER){
            cardButton.setText("<html>" + card.getColour() + "<br>" + card.getNumberAsWord() + "</html>");

            if(card.getColour().equals(Card.Colour.RED)){
                cardButton.setBackground(Color.RED);
            }
            else if(card.getColour().equals(Card.Colour.GREEN)){
                cardButton.setBackground(Color.GREEN);
            }
            else if(card.getColour().equals(Card.Colour.YELLOW)){
                cardButton.setBackground(Color.YELLOW);
            }
            else if(card.getColour().equals(Card.Colour.BLUE)){
                cardButton.setBackground(Color.BLUE);
            }
        }
        else{
            cardButton.setText("<html>" + card.getColour() + "<br>" + card.getCardType() + "</html>");
            if(card.getColour().equals(Card.Colour.RED)){
                cardButton.setBackground(Color.RED);
            }
            else if(card.getColour().equals(Card.Colour.GREEN)){
            ...
            else if(card.getColour().equals(Card.Colour.NONE)){
    ...
    public void initializeButtonHand() {
        ...
        for (UnoPlayer currentPlayer : player) {
            ...
            for (int i = 0; i < usersCards.size(); i++) {
                ...
                b.addActionListener(new cardClickerListener(model, event, this));
                buttonPanel.add(b);
                cardButtons.add(b);
                updateCards();
                //cardGive();
    ...
    public void enableLegalButtons () {
        ...
        for (int i=0; i<usersHand.getCards().size();i++) {
            if (usersHand.isCardPlayable(usersHand.getCards().get(i),model.getCurrentCard(),model.getChosenColour())) {
